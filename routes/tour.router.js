...
router.get("/getlist", checkApikey, async (req, res) => {
...
router.get("/getbyfaverite", checkApikey, async (req, res) => {
    const { _limit } = req.query;
    const tour = await ModelTour.find({}).limit(_limit);
    res.send(tour)
})

router.get("/getbyexpensivetour", checkApikey, async (req, res) => {
    const { _limit } = req.query;
...
router.get("/getbycheaptour", checkApikey, async (req, res) => {
    const { _limit } = req.query;
    const tour = await ModelTour.find({}).sort({ price: 1 }).limit(_limit);
    res.send(tour)
})
